  ...
  presets: [
    ...
    presetIcons({
      ...
      collections: {
        ...
        'ph': async (name) => {
          const style = name.includes('-duotone') ? 'duotone' :
                      name.includes('-light') ? 'light' :
                      name.includes('-bold') ? 'bold' :
          ...
          try {
            const response = await fetch(
              `https://api.iconify.design/ph/${baseName}.svg?style=${style}`
            );
            if (!response.ok) throw new Error();
            const data = await response.text();
            return data.replace(/^<svg\s/, '<svg fill="currentColor" ');
          } catch (e) {
            console.error(`Failed to load icon: ${name} (${style}/${baseName})`);
            return '';
          }
        },
        'svg-spinners': async (name) => {
          try {
            const response = await fetch(
              `https://api.iconify.design/svg-spinners/${name}.svg`
            );
            if (!response.ok) throw new Error();
            const data = await response.text();
            return data.replace(/^<svg\s/, '<svg fill="currentColor" ');
